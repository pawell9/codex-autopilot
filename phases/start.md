...
3. Resolve `<control-root>/.autopilot/owner.lock` and the run path `<control-root>/.autopilot/runs/<run-id>`. Existing nonterminal state is recovered, never overwritten. A terminal run is read-only and a changed scope creates a successor run.
4. Validate schema, current ledger revision, owner token/epoch, referenced document hashes, pending operations, leases, and `next_action`. Use `tools/ledger.py status` or `brief`; do not hand-edit JSON.
...
On resume, enter `RECOVERING` before product dispatch. Reconcile current and previous ledger publications, actual Git state, operations, inboxes, handles/process observations, instructions, capability invalidation, and owner epoch. A missing UI handle is not proof of stop. Use `phases/recover.md` for uncertain liveness. Then set the earliest invalid phase and an exact durable `next_action`; only after reconciliation return to `ACTIVE`.

**Done when:** the exact repository, namespace, owner, phase/control, capabilities, authority, baseline, and next safe action are durable, or the run has a precise `BLOCKED_PREFLIGHT`/`BLOCKED` diagnostic with no product dispatch.
