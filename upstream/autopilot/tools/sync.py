...
Делает четыре вещи, в этом порядке:
  ...
  2. Вписывает состояние внутрь dashboard.html между маркерами — атомарно, через
     ...
     прогона.
  3. Сверяет статусы canonical ticket Markdown с authoritative state.
  4. Смотрит, жив ли статический сервер этого прогона, и поднимает на прежнем
...
TICKET_STATUS = {
    # The legacy state uses ``pending`` for a ticket that has been cut, while
    # its canonical Markdown uses ``ready`` for the same state.
    "pending": "ready",
...
def is_ours(cmd):
    """Наш ли это process, проверяя разобранные argv, а не подстроку."""
    try:
        argv = shlex.split(cmd)
    except ValueError:
        return False
    try:
        module_at = argv.index("-m")
        directory_at = argv.index("--directory")
    except ValueError:
        return False
    return (
        module_at + 1 < len(argv)
        and argv[module_at + 1] == "http.server"
        and directory_at + 1 < len(argv)
        and os.path.abspath(argv[directory_at + 1]) == os.path.abspath(A)
    )


def recorded():
    try:
        with open(PIDF, encoding="utf-8") as stream:
            port, pid = stream.read().split()
...
def process_info(pid):
    """Return (state, command) for a PID, or None when it is gone."""
...
def process_running(pid):
    """Check termination, treating a zombie as stopped."""
    info = process_info(pid)
    return bool(info and not info[0].startswith("Z"))


def owned_pids(recorded_pid=None):
    """Find only this run's dashboard servers, including stale pid records."""
    found = set()
    if recorded_pid and is_ours((process_info(recorded_pid) or ("", ""))[1]):
        found.add(recorded_pid)
    try:
    ...
    for line in output.splitlines():
        ...
        if number.isdigit() and is_ours(command):
            found.add(int(number))
    return sorted(found)


def stop_dashboard(grace=2.0):
    """Stop this run's dashboard and verify that every owned PID is gone.

    A missing, malformed, stale, or already-dead pid file is normal during
    resume/recovery and is deliberately treated as an idempotent no-op.  A
    live process is matched by its exact ``http.server --directory`` argv
    before any signal is sent; PID reuse therefore cannot stop another app.
    """
    _, recorded_pid = recorded()
    pids = owned_pids(recorded_pid)
    for pid in pids:
        try:
            ...
            # The process may have disappeared between ps and kill.  The
    ...
    deadline = time.monotonic() + max(0.0, grace)
    while any(process_running(pid) for pid in remaining) and time.monotonic() < deadline:
        time.sleep(0.05)
    still_running = [pid for pid in remaining if process_running(pid)]
    if still_running:
        raise RuntimeError("dashboard server did not terminate: %s" % ", ".join(map(str, still_running)))

    # Remove only this run's bookkeeping.  It is safe to repeat after a
    # successful cleanup and useful when a PID was stale or missing.
    try:
        os.unlink(PIDF)
    except FileNotFoundError:
        pass
    except OSError:
        # A read-only/moved pid file is not evidence that a server survived.
        # The verified process result above is the acceptance condition.
        pass
    return pids
...
def serve(state):
    if state.get("finishedAt"):
        stop_dashboard()
        return "прогон закрыт — сервер остановлен"
...
def ticket_artifact_path(ticket_root, ticket_id):
    """Resolve exactly one canonical Markdown artifact for a ticket ID."""
    matches = []
    for path in ticket_root.iterdir():
        if path.suffix != ".md":
            continue
        if path.name == "%s.md" % ticket_id or path.name.startswith("%s-" % ticket_id):
            if path.is_symlink():
                raise RuntimeError("canonical ticket artifact may not be a symlink: %s" % path)
            if not path.is_file():
                continue
            matches.append(path)
    if len(matches) != 1:
        raise RuntimeError("expected one canonical ticket artifact for %s, found %d" % (ticket_id, len(matches)))
    return matches[0]


def sync_ticket_artifacts(state):
    """Project authoritative ticket state into canonical ticket Markdown.

    ``state.js`` remains authoritative.  The Markdown files are updated only
    in their single Status field, with an atomic replacement, and a complete
    plan is validated before the first write.  Re-running this function after
    pause/resume or terminal recovery is therefore safe and idempotent.
    """
    tickets = state.get("tickets") or []
    if not tickets:
        return []
    ticket_root = Path(os.path.abspath(os.path.join(A, state.get("dir", ""), "tickets")))
    if os.path.commonpath([os.path.abspath(A), str(ticket_root)]) != os.path.abspath(A):
        raise RuntimeError("ticket artifact directory escapes .autopilot")
    if not ticket_root.is_dir() or ticket_root.is_symlink():
        raise RuntimeError("canonical ticket artifact directory is unavailable: %s" % ticket_root)

    plan = []
    status_pattern = re.compile(r"(?m)^(\*\*Status:\*\*)[ \t]*[^\r\n]*$")
    for ticket in tickets:
        ticket_id = str(ticket.get("id", ""))
        artifact = ticket_artifact_path(ticket_root, ticket_id)
        raw = artifact.read_bytes()
        text = raw.decode("utf-8")
        if len(status_pattern.findall(text)) != 1:
            raise RuntimeError("canonical ticket artifact must contain one Status line: %s" % artifact)
        desired = TICKET_STATUS.get(ticket.get("status"))
        if desired is None:
            raise RuntimeError("unsupported authoritative ticket status: %s" % ticket.get("status"))
        updated = status_pattern.sub(r"\1 " + desired, text, count=1)
        plan.append((artifact, raw, updated.encode("utf-8")))

    changed = []
    for artifact, raw, updated in plan:
        if raw == updated:
            continue
        mode = stat.S_IMODE(artifact.stat().st_mode)
        fd, temporary = tempfile.mkstemp(prefix=".%s." % artifact.name, suffix=".tmp", dir=ticket_root)
...
def main():
    ...
    synced_tickets = sync_ticket_artifacts(state)
    if passed:
        save(state)                    # updatedAt не двигаем: пульс принадлежит агенту
    snap = write_snapshot(state)
    # ``--no-serve`` suppresses startup for headless/remote runs, but it must
    # never suppress terminal cleanup: an existing detached server is still a
    # resource owned by this run and must be stopped and verified.
    srv = serve(state) if state.get("finishedAt") else ("сервер не проверялся" if "--no-serve" in sys.argv else serve(state))
    print("%s · %s · обновлено %s" % (snap, srv, (state.get("updatedAt") or "?")[11:19]))
    for line in passed:
        print("  · " + line)
    for path in synced_tickets:
        print("  · canonical ticket synced: %s" % path)
