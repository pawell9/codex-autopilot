...
**Sync once more right after writing it** — `python3 .autopilot/sync.py`. This is the write that decides what a landed run looks like six months later: the snapshot inside `dashboard.html` freezes on the final numbers, the canonical ticket Markdown is reconciled from the authoritative ticket rows, and the page opens from the archive with the whole flight intact. For a terminal state `sync.py` also performs the dashboard shutdown and verifies that every server for this run has exited; it does not merely send a stop signal.

**Then, and only then, put out the server** — this is part of the same `sync.py` terminal finalization, after the final snapshot is written. It goes last because the page has to receive the final state first, and it goes at all because a run that ends leaving an HTTP server on the user's machine has left something running that nobody will ever think to stop.

**The terminal `sync.py` cleanup is idempotent.** It checks the recorded PID and scans for additional `python -m http.server` processes whose parsed `--directory` argument is exactly this run's `.autopilot` path. Missing, stale, or already-dead PIDs are treated as successful absence; live matching processes are signalled, waited on, and rechecked before the PID bookkeeping is removed. A process that remains alive is an explicit finalization error, not a green acceptance.
