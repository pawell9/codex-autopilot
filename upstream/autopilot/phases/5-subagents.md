...
8. **Update the instruments** (`phases/7-instruments.md`) — one line of state, one line of the dashboard: the ticket's `finishedAt`, tests and commit, the `requirements` counts, the `build` and `review` stage notes («3 из 5 тасков готовы»), `updatedAt`. The same `sync.py` call also projects every authoritative ticket status into its canonical Markdown artifact; do not hand-edit a conflicting status or leave the artifact at `ready` after a ticket is done.
