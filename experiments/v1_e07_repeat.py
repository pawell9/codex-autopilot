...
def manual_microproject(root: Path, name: str, output: str, verdict: str) -> dict[str, object]:
    ...
    receipt = root / f"{name}-commit.json"; write_json(receipt, {"status": "PASS", "run_id": run_id, "ticket_id": "T-1", "attempt_id": "A-1", "operation_id": "OP-1", "kind": "candidate_commit", "target": str(repo), "checkout": str(repo), "expected_before": base, "base_sha": base, "intended_after": candidate_sha, "commit_sha": candidate_sha, "tree_sha": tree_sha, "authority_ref": "E07", "receipt_ref": "E07-commit"})
